...
public class TransportAPI {
	...
	private String getDepartureTime(String isoDate) {
		try {
			...
			//Differenz zwischen aktueller Uhrzeit und Abfahrtszeit berechnen und 
			//diese von Millisekunden zu Minuten rechnen
			int departureInMinutes = ((int) Math.ceil(((double) departureDate.getTime() - (double) currentDate.getTime()) / (1000*60) ));
			
			
			
			//Wenn Abfahrt in unter einer Stunde, nur die Minuten zurï¿½ckgeben (Modulo 60 von der Abfahrt in Minuten)		
			if (departureInMinutes / 60 == 0) {
				return String.valueOf(departureInMinutes % 60);
			}
			//Sonst die Minuten zu Stunden umwandeln und mit den restlichen Minuten (Modulo 60 von der Abfahrt in Minuten)
			//konkatinieren
			else {
	...
	public StationBoard getStationBoard(String id) {
		...
		JSONObject mainJSONObject = new JSONObject(transportAPICall.getResponse("http://transport.opendata.ch/v1/stationboard?station=" + id + "&limit=12"));
		...
		for (int i = 0; i < jsonStationBoard.length(); i++) {
			...
			stationBoard.addItems(jsonStartStation.getString("name"),
								  jsonStationBoardInstance.getString("to"), 
								  getDepartureTime(jsonDeparture.getString("departure")),
							      jsonStationBoardInstance.getString("name"),
							      jsonCoordinate.getDouble("x"),
							      jsonCoordinate.getDouble("y"));
	...
	public int getID(String name) {		
		...
		//Erstes Wort der Benutzereingabe speichern fï¿½r API Call
		String test[] = name.split(" ");
		
		
		JSONObject jsonMainObject = new JSONObject(transportAPICall.getResponse("http://transport.opendata.ch/v1/locations?query=" + test[0].toString()));
		...
		for (int i = 0; i < jsonStations.length(); i++) {
			...
			//Wenn die gesuchte Haltestelle mit einer von der API zurï¿½ckgelieferten Haltestelle ï¿½bereinstimmt
		...
		//Wenn keine Haltestelle genau mit den Ergebnissen ï¿½bereinstimmt, die erste Haltestelle zurï¿½ckliefern
	...
	public StationBoard getConnections(String from, String to) {
		...
		JSONObject jsonMain = new JSONObject(transportAPICall.getResponse("http://transport.opendata.ch/v1/connections?from=" + from + "&to=" + to +"&limit=13"));
		JSONArray  jsonConnections = jsonMain.getJSONArray("connections");
		stationBoard = new StationBoard();
		
		//Bei zweitem Element in Array beginnen, da die TransportAPI immer eine bereits vergangene Verbindung liefert
		for (int i = 1; i < jsonConnections.length(); i++) {
			...
			stationBoard.addItems(jsonStation.getString("name"), 
								  jsonStationTo.getString("name"),
								  getDepartureTime(jsonFrom.getString("departure")),
								  jsonSectionsInstance.getJSONObject("journey").getString("name"),
								  jsonCoordinate.getDouble("x"),
								  jsonCoordinate.getDouble("x"));
